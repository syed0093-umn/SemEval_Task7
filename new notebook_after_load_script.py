...
def load_data(fact_checks_path, posts_path, pairs_path, tasks_path):
    ...
    with open(tasks_path) as f:
        tasks = json.load(f)
    return fact_checks, posts, pairs, tasks

# %%
def preprocess_text(row):
    """Combine relevant text fields for comparison."""
    texts = []
    ...
    if isinstance(row.get('ocr'), str):
        ocr_data = eval(row['ocr'])
        if isinstance(ocr_data, list):
            texts.extend([text for item in ocr_data if isinstance(item, dict) 
                        for text in item.values() if isinstance(text, str)])
    
    return ' '.join(texts)

# %%
def create_retrieval_system(df_fact_checks, df_posts, task_config, language):
    ...
    fact_checks = df_fact_checks[df_fact_checks['fact_check_id'].isin(valid_fact_check_ids)]
    
    # Prepare fact check texts
    fact_check_texts = []
    for _, row in fact_checks.iterrows():
        claim_data = eval(row['claim'])
        title_data = eval(row['title'])
        texts = [text for item in [claim_data, title_data] 
                for text in item.values() if isinstance(text, str)]
...
def generate_predictions(posts_df, task_config, vectorizer, fact_check_vectors, 
    ...
    for post_id in valid_post_ids:
        ...
        post_text = preprocess_text(post)
...
def main():
    ...
    # Save predictions
    with open('predictions/monolingual_predictions_copy.json', 'w') as f:
        json.dump(all_predictions, f)
...
def main():
    ...
    fact_checks = pd.read_csv('fact_checks.csv')
    posts = pd.read_csv('posts.csv')
    pairs = pd.read_csv('pairs.csv')  # Load gold pairs
    with open('tasks.json') as f:
        tasks = json.load(f)
    ...
    print("\nSaving predictions...")
    with open('predictions/monolingual_predictions_bert_2.json', 'w') as f:
        json.dump(all_predictions, f)
...
def main():
    ...
    with open('predictions/monolingual_predictions.json', 'w') as f:
